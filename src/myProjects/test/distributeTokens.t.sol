...
import {DistributeTokens} from "../src/DistributeTokens/distributeTokens.sol";

contract DistributeTokensTest is Test {
    //     address[] public arr=[
    ...
    //     ];
    address[] public arr = [address(0), address(1), address(2)];

    DistributeTokens public dp;

    function setUp() public {
        dp = new DistributeTokens();
    ...
    function testDistributeTokens() public {
        ...
        uint256 initialContractBalance = address(dp).balance;

        uint256 amount = 30;
        dp.distributeTokens(amount);

        uint256 expectedBalance = initialContractBalance + amount;
        assertEq(
            address(dp).balance,
            expectedBalance,
            "Contract balance should increase by amount"
        );
        for (uint i = 0; i < arr.length; i++) {
            assertEq(arr[i].balance, amount / arr.length, "Balance of receiver is incorrect");
